...
<div class="admin-card">
    <div class="admin-card-header"><h5>Edit: {{ $portfolio->title }}</h5></div>
        ...
        <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control @error('category') is-invalid @enderror" name="category" value="{{ old('category', $portfolio->category) }}">
                ...
                <div class="col-md-6">
                    ...
                    @if($portfolio->image)
                        <div class="mt-2">
                            <img src="{{ asset($portfolio->image) }}" alt="Current" class="img-thumbnail" id="imgPreview" style="max-height: 150px;">
                        </div>
                    @else
                ...
                <div class="col-md-6">
                    ...
                    <input type="url" class="form-control @error('link') is-invalid @enderror" name="link" value="{{ old('link', $portfolio->link) }}">
                    @error('link')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4 d-flex align-items-end">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $portfolio->is_active) ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_active">Active</label>
                    </div>
                </div>
                <div class="col-12">
                    <button type="submit" class="btn btn-primary-custom"><i class="bi bi-check-lg me-1"></i> Update Portfolio</button>
                </div>
            </div>
        </form>
    </div>
